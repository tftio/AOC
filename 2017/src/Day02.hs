...
choose _ 0  = [[]]
choose [] _ = []
choose (x:xs) n = fmap (x:) (choose xs (n - 1)) ++ choose xs n
...
part2 r =
  if a > b then a `div` b else b `div` a
  where
    isValidPair a b = (b `mod` a == 0) || (a `mod` b == 0)
    (a:b:_) = (head (filter (\(a:b:_) -> isValidPair a b) (r `choose` 2)))

checkSumOfSpreadsheet f = sum . map f
